...
public class PetViewModelConverter {
    ...
    public static PetViewModel convert(Pet pet) {
        ...
        OwnerViewModel[] otherOwners;
        if (pet.getOtherOwners()!=null && pet.getOtherOwners().length > 0) {
            otherOwners = new OwnerViewModel[pet.getOtherOwners().length];
            for (int i = 0; i < pet.getOtherOwners().length; i++) {
                otherOwners[i] = OwnerViewModelConverter.convert(pet.getOtherOwners()[i]);
            }
            petViewModel.setOtherOwners(otherOwners);
        }
